...
export default function PairsSlide({ students, onReward }: PairsSlideProps) {
  ...
  const [selectedPrompt] = useState<SpeakingPrompt>(PAIRS_PROMPTS[0]);
  const [activePair, setActivePair] = useState<Pair | null>(null);
  const [isProjecting, setIsProjecting] = useState(false);

  // Logic to pair up all students randomly
  const generatePairs = () => {
  ...
  return (
    <div id="pairs-slide" className="grid grid-cols-1 lg:grid-cols-12 gap-4 w-full">
          ...
          {pairs.length === 0 ? (
            <div className="text-center py-5">
              ...
              <p className="text-[10px] text-stone-600 max-w-[200px] mx-auto mt-0.5 font-bold">
                Tap to pair our 16 students into 8 roleplay booths.
              </p>
            </div>
          ) : (
      ...
      {/* Right Column: Main Roleplay Arena */}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[9px] font-mono font-black text-black/55 tracking-wider uppercase border border-black px-1.5 bg-neo-yellow/30">
                    ACTIVITY 2: PAIRS SPEAKING
                  </span>
                </div>
              </div>
            </div>

            {/* Active Pair Spotlight */}
            {activePair ? (
              ...
              <div className="p-4 border-4 border-dashed border-black bg-stone-50 text-center mb-3">
                ...
                <p className="text-[11px] font-black uppercase tracking-wider text-black mt-1">No Speakers Assigned Yet</p>
                <p className="text-[9px] text-stone-500 font-bold">Tap "Create Match Pairs" on the left side!</p>
              </div>
            )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-sans font-bold text-black tracking-tight">
                        {activePair && isA 
                          ? line.replace('A:', `[${activePair.studentA.name}]:`)
                          : activePair && isB 
                          ? line.replace('B:', `[${activePair.studentB.name}]:`)
                          : line
                        }
                      </span>
                    </div>
            ...
            <div className="mt-3 bg-[#00FF00]/10 border-2 border-black p-2.5 flex flex-wrap items-center justify-between gap-2 shadow-neo-sm animate-scale-up">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[9px] font-bold text-stone-600">Give stars on the scoreboard</p>
                </div>
              </div>
              <button
                onClick={() => {
      ...
      {isProjecting && (
        <div className="fixed inset-0 z-[100] bg-white border-8 border-black flex flex-col justify-between p-8 text-black animate-scale-up">
          ...
          <div className="flex-1 flex flex-col justify-center items-center max-w-5xl mx-auto w-full py-8 text-center">
            ...
            {activePair && (
              <div className="mb-6 bg-neo-yellow border-4 border-black p-4 inline-flex items-center gap-8 shadow-neo">
                <div className="flex items-center gap-2">
                  <span className="text-4xl">{activePair.studentA.emoji}</span>
                  <span className="font-display font-black text-xl uppercase">{activePair.studentA.name}</span>
                </div>
                <span className="text-2xl font-black">🤝</span>
                <div className="flex items-center gap-2">
                  <span className="text-4xl">{activePair.studentB.emoji}</span>
                  <span className="font-display font-black text-xl uppercase">{activePair.studentB.name}</span>
                </div>
              </div>
            )}
            ...
            <div className="w-full space-y-4 my-auto">
              {selectedPrompt.guidelines?.map((line, idx) => {
                ...
                return (
                  <div 
                    key={idx}
                    className={`p-6 border-4 border-black font-display text-2xl md:text-3xl font-black text-black flex items-center gap-4 shadow-neo text-left ${
                      isA ? 'bg-neo-blue/15' : isB ? 'bg-neo-coral/15' : 'bg-slate-50'
                    }`}
                  >
                    <span className={`text-lg font-mono font-black w-10 h-10 rounded-none flex items-center justify-center shrink-0 border-2 border-black ${
                      isA ? 'bg-neo-blue text-white' : isB ? 'bg-neo-coral text-white' : 'bg-black text-white'
                    }`}>
                      {isA ? 'A' : isB ? 'B' : '?'}
                    </span>
